"""Name-en-US: Create RVA3D Diorama Template
Description-en-US: Build a new, unsaved canonical RVA3D diorama document.
...
def create_box_mesh(
    ...
    # Outward-facing quads: front, back, left, right, top, bottom.
    polygons = (
...
def create_portal_aperture() -> c4d.PolygonObject:
    """Create the single-quad, +Z-facing portal surface."""
    ...
    # Counter-clockwise from the +Z side produces a +Z normal.
    aperture.SetPolygon(0, c4d.CPolygon(0, 1, 2, 3))
...
def build_document() -> tuple[c4d.documents.BaseDocument, c4d.BaseObject]:
    ...
    for name, z_position in (
        ...
        ("DEPTH_CUBE_BACK_Z_NEG_200", -200.0),
        ("DEPTH_DEEP_Z_NEG_500", -500.0),
        ("DEPTH_EXTREME_Z_NEG_1000", -1000.0),
    ):
        insert_child(
            guide_depth,
    ...
    camera.SetRelRot(c4d.Vector(0.0, math.pi, 0.0))
    ...
    camera[c4d.CAMERAOBJECT_USETARGETOBJECT] = True
    camera[c4d.CAMERAOBJECT_TARGETOBJECT] = portal_target
...
def completion_summary() -> str:
    ...
    return "\n".join(
        (
            "RVA3D diorama template created in a NEW, unsaved document.",
            ...
            "Save the resulting Cinema 4D document manually.",
        )
    )
...
def main() -> None:
    ...
    try:
        ...
        print(details)
        gui.MessageDialog(
            "RVA3D diorama template creation failed.\n\n"
            f"{error}\n\n"
