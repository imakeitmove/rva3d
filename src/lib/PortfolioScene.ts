...
// Local interaction state specifically for the 3D scene
type InteractionState3D = {
  mouse: {
...
export class PortfolioScene {
  ...
  async loadPortfolio(items: NotionPortfolioItem[]) {
    ...
    items.forEach((item, i) => {
      ...
      const window = new ContentWindow3D(item, new THREE.Vector3(x, y, z));
  ...
  private updateIdleOrbit(deltaTime: number) {
    ...
    this.windows.forEach((window, i) => {
      ...
      const direction = lookAtVector
        .sub(window.targetObject.position)
        .normalize();
    ...
    this.logoGroup.children.forEach((child, i) => {
      if (i > 0) {
        // Skip the sphere, just rotate rings
  ...
  private getInteractableMeshes(): THREE.Object3D[] {
    return this.windows
      .map((w) => w.cubeMesh)
  ...
  private handleMouseDown() {
    ...
    const intersects = this.raycaster.intersectObjects(
      this.getInteractableMeshes()
    );

    if (intersects.length > 0) {
      const window = intersects[0].object.userData
        .contentWindow as ContentWindow3D;
      if (window) {
        window.onDragStart();
  ...
  update() {
    ...
    if (this.sceneState === "active") {
      ...
      this.windows.forEach((w) => {
        const mesh = w.cubeMesh;
        if (
          w.isHovered &&
          (!mesh || !intersects.find((i) => i.object === mesh))
        ) {
          w.onHoverEnd();
        }
      });
