...
import { PRIVATE_REVIEW_SESSION_SECONDS, verifyPrivateReviewToken, privateReviewTokenScope, type ReviewScope } from "./review-session";
export { createPrivateReviewToken, isPrivateReviewPassword, verifyPrivateReviewToken, privateReviewTokenScope, reviewScopeForPath, reviewConfigured } from "./review-session";
export const PRIVATE_REVIEW_COOKIE = "rva3d_private_review_session";
export { PRIVATE_REVIEW_SESSION_SECONDS };
export function privateReviewCookieOptions() {
  return { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax" as const, path: "/review", maxAge: PRIVATE_REVIEW_SESSION_SECONDS };
}
export async function hasPrivateReviewSession(requiredScope: ReviewScope = "all") {
  const token = (await cookies()).get(PRIVATE_REVIEW_COOKIE)?.value;
  return token ? verifyPrivateReviewToken(token, Date.now(), requiredScope) : false;
}
export async function privateReviewSessionScope() {
  const token = (await cookies()).get(PRIVATE_REVIEW_COOKIE)?.value;
  return token ? privateReviewTokenScope(token) : null;
}
export async function requirePrivateReviewSession(pathname = "/review", requiredScope: ReviewScope = "all") {
  if (!(await hasPrivateReviewSession(requiredScope))) redirect("/review/login?next=" + encodeURIComponent(safeReviewNext(pathname)));
}
export function safePrivateReviewPath(value: FormDataEntryValue | null) { return safeReviewNext(value); }

// Superseded shared-only v1 sessions are intentionally revoked by the scoped v2 gate.
// import "server-only";
// import { safeReviewNext } from "./review-boundary";
//
// import {
