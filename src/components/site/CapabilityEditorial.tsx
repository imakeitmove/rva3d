...
export function CapabilityEditorial() {
  ...
  const desmi = protectedMedia({ kind: "video", presentation: "loop", src: "/media/capabilities/desmi-chocolate-pump-loop.mp4", width: 1280, height: 720, mimeType: "video/mp4", alt: "DESMI Rotan chocolate pump technical visualization", poster: { kind: "image", src: "/media/capabilities/desmi-chocolate-pump-poster.webp", width: 1280, height: 720, alt: "DESMI Rotan chocolate pump cutaway" } } as WorkVideoMedia);
  ...
  return <Shell><div className="capabilities-editorial capabilities-v2">
    ...
    <div className="editorial-width capability-wayfinding" data-tone="paper"><nav aria-label="Jump to a capability">{capabilities.map(capability => <a key={capability.slug} href={`#${capability.slug}`}>{capability.title}<span aria-hidden="true">&#8599;</span></a>)}</nav></div>
    <div className="refined-capabilities">{capabilities.map(capability => {
      const copy = capabilityEditorial[capability.slug];
      if (capability.slug === "interactive-3d") return <section className="interactive-also" id={capability.slug} key={capability.slug} data-tone="paper"><div className="editorial-width"><div className="capability-section-heading"><p className="label">Also / Interactive Media &amp; Prototyping</p><h2>{copy.promise}</h2></div><div className="interactive-proof-grid"><InteractiveLogo modelUrl={logoModelUrl} /><div><p>{copy.copy}</p><a className="proof-link" href={siteHref("/interactive")}>Explore this site&#8217;s interactive gallery <span aria-hidden="true">&#8599;</span></a><p className="studio-note">Self-initiated RVA3D work.</p></div></div></div></section>;
      const media = selected[capability.slug];
      return <section className={`refined-capability refined-${capability.slug}`} key={capability.slug} id={capability.slug} data-tone={capability.slug === "product-technical-visualization" ? "void" : "paper"}><div className="editorial-width"><div className="capability-section-heading"><p className="label">{capability.title}</p><h2>{copy.promise}</h2></div><div className="capability-proof-grid"><div className="large-capability-proof">{media.kind === "video" ? <CapabilityPlayer media={media} destination={capability.slug === "3d-animation" ? { href: siteHref("/work/wawa-coffee-island"), label: "Open the Wawa Coffee Island case study" } : undefined} /> : <SiteMedia capabilityFullscreen media={media} />}</div><div className="capability-commission"><p>{copy.copy}</p><ul className="need-labels" aria-label="Good for">{copy.uses.map(need => <li key={need}>{need}</li>)}</ul><div className="capability-proof-note"><p>{copy.proof}</p>
        {capability.slug === "3d-animation" && <a className="proof-link" href={siteHref("/work/axe-whaxe-lil-baby")}>See the AXE WHAXE story <span aria-hidden="true">&#8599;</span></a>}
        {capability.slug === "product-technical-visualization" && <><a className="proof-link" href={siteHref("/work/amsoil-xpd-wind-grease")}>See the AMSOIL story <span aria-hidden="true">&#8599;</span></a><a className="proof-link supporting-proof" href={siteHref("/work/wawa-coffee-island")}>From fixture CAD to a stocked Wawa Coffee Island <span aria-hidden="true">&#8599;</span></a></>}
    ...
    <section className="messy-brief" data-tone="paper"><div className="editorial-width messy-grid"><div><p className="label">A good place to start</p><h2>Bring the<br /><em>messy version.</em></h2></div><div><p>A finished brief is welcome, but not required. Bring the script, sketch, CAD file, reference folder, footage or half-formed idea.</p><p>We&#8217;ll work out what needs making, what to review when and the clearest path to final.</p><div className="messy-actions"><a className="button" href={siteHref("/#contact")}>Get in touch <span aria-hidden="true">&#8599;</span></a><a className="proof-link" href={siteHref("/about#how-we-work")}>See how we work <span aria-hidden="true">&#8599;</span></a></div></div></div></section>
  </div></Shell>;
}
