...
// Public-safety audit: previous content retained for reference.
// "<figure><SiteMedia capabilityFullscreen media={proof.lotteryBefore} /><figcaption>Original</figcaption></figure>"
  ...
  return <Shell><div className="capabilities-editorial capabilities-v2">
    ...
    <div className="refined-capabilities">{capabilities.map(capability => {
      ...
      return <section className={`refined-capability refined-${capability.slug}`} key={capability.slug} id={capability.slug} data-tone={capability.slug === "product-technical-visualization" ? "void" : "paper"}><div className="editorial-width"><div className="capability-proof-grid"><div className="capability-section-heading"><p className="label">{capability.title}</p><h2>{copy.promise}</h2></div><div className="large-capability-proof">{media.kind === "video" ? <CapabilityPlayer media={media} /> : <SiteMedia capabilityFullscreen media={media} />}<ul className="need-labels" aria-label="Good for">{copy.uses.map(need => <li key={need}>{need}</li>)}</ul></div><div className="capability-commission">{copy.copy.split("\n\n").map(paragraph => <p key={paragraph}>{paragraph}</p>)}{/* Supporting details previously appeared here as boxed labels; now beneath the related media. */}<div className="capability-proof-note">{copy.proof && <p>{copy.proof}</p>}
        ...
        {capability.slug === "vfx-compositing" && <><a className="proof-link" href={siteHref("/capabilities/vfx-compositing")}>A closer look at VFX &amp; compositing <span aria-hidden="true">&#8599;</span></a><details className="vfx-microcase"><summary>Virginia Lottery / A quieter replacement</summary><div className="microcase-pair">{/* Project-specific authorization restores the original/finished comparison. */}<figure><SiteMedia capabilityFullscreen media={proof.lotteryBefore} /><figcaption>Original</figcaption></figure><figure><SiteMedia capabilityFullscreen media={proof.lotteryAfter} /><figcaption>Finished sign</figcaption></figure></div><p>The replacement sign follows the original perspective and light.</p></details></>}
