...
const publicRoutePattern = /^\/(?:$|work(?:\/[^/]+)?\/?$|capabilities(?:\/[^/]+)?\/?$|interactive\/?$|about\/?$|hello\/?$|privacy\/?$|login(?:\/recovery)?\/?$|client-login\/?$|contact\/?$)/;
...
export function proxy(request: NextRequest) {
  ...
  const finishPublic = (response: NextResponse) => finish(response, publicHeaders);
