...
export default async function ProjectExperience({
  ...
  const [portalPage, project, posts] = await Promise.all([
    getPortalPageByClientId(userId),
    getProjectPageForClientProject({ userId, projectId }),
    getPostsForProject(userId, projectId),
  ]);
  ...
  if (!posts.length) {
    return (
      <main className="project-shell">
        <a className="breadcrumb" href={`/portal/${userId}`}>
          ← Back to portal
        </a>
  ...
  const activePost = selectedSlug
    ? await getPostWithContentForProject({ userId, projectId, postSlug: selectedSlug })
    : null;
      ...
      <section className="history-section">
        ...
        <div className="history-grid">
          {posts.map((post) => {
            ...
            const href = `/portal/${userId}/projects/${projectId}/posts/${post.postId}`;
            return (
              <a
                key={post.postId}
                href={href}
