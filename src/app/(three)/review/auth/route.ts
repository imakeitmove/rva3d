...
import { createPrivateReviewToken, isPrivateReviewPassword, PRIVATE_REVIEW_COOKIE, privateReviewCookieOptions, reviewScopeForPath, reviewConfigured } from "@/lib/private_review_auth";
...
export async function POST(request: Request) {
  ...
  // Configuration is checked against the requested project below.
  ...
  const requestedScope = reviewScopeForPath(next);
  if (requestedScope !== "all" && !permissionReviewAvailable(requestedScope)) return new Response(null, { status: 404, headers: privateHeaders });
  if (!reviewConfigured(requestedScope) && !reviewConfigured("all")) return new Response("Private review is unavailable.", { status: 503, headers: privateHeaders });
  const candidate = form.get("password") || "";
  const scope = isPrivateReviewPassword(candidate) ? "all" : requestedScope;
  const valid = isPrivateReviewPassword(candidate, scope);
  ...
  if (valid) response.cookies.set(PRIVATE_REVIEW_COOKIE, createPrivateReviewToken(Date.now(), scope), privateReviewCookieOptions());
  return response;
}
