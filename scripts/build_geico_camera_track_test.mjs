...
const definitions = [
  { id: "cameraTrackTest", source: "intro_shot_render_stills/geico_geckOs_box_anim_shot1_cameraTrack_V02_Main0018.png", name: "geico_early_camera_tracked_box_test", widths: [800, 1200, 1920], quality: 90, caption: "Early camera-tracked box animation test.", alt: "Early box animation test frame with a bent cereal box on a simple tabletop.", statusLabel: "Early camera-tracked box animation test." },
];
...
await fs.writeFile(path.join(repo, "src/content/site/geico_camera_track_test.generated.json"), JSON.stringify(media, null, 2) + "\n");
...
await fs.writeFile(path.join(projectRoot, "geico_camera_track_test_media_manifest.json"), JSON.stringify(report, null, 2) + "\n");
await fs.copyFile(fileURLToPath(import.meta.url), path.join(projectRoot, "build_geico_camera_track_test.mjs"));
