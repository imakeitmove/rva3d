...
const routeNames = ["media/[key].func", "review/assets/[key].func"];
for (const routeName of routeNames) {
  const route = path.join(functions, routeName);
  assert((await fs.stat(route)).isDirectory(), `Vercel media function is missing: ${routeName}`);
  const functionConfig = JSON.parse(await fs.readFile(path.join(route, ".vc-config.json"), "utf8"));
  assert.equal(functionConfig.runtime, "nodejs22.x", `Unexpected media-function runtime: ${routeName}`);
  for (const [key, entry] of Object.entries(manifest)) {
    const bytes = await fs.readFile(path.join(route, "private-media", key));
    assert.equal(bytes.length, entry.bytes, `Function byte length mismatch: ${routeName}/${key}`);
    assert.equal(digest(bytes), entry.sha256, `Function SHA mismatch: ${routeName}/${key}`);
  }
  const privateFiles = (await fs.readdir(path.join(route, "private-media"))).sort();
  assert.deepEqual(privateFiles, Object.keys(manifest).sort(), `Unexpected private files in ${routeName}`);
}
...
console.log({ status: "PASS", functionAssets: Object.keys(manifest).length, functionPaths: routeNames, privateStaticCopies: 0 });
